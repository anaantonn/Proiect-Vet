...
class DeparazitareViewSet(viewsets.ModelViewSet):
    ...
    def update(self, request, pk=None):
        try:
            ...
            serializer = self.get_serializer(deparazitare, data=request.data, partial=True)
            if serializer.is_valid():
                serializer.save()
                return Response(serializer.data, status=status.HTTP_201_CREATED)
            return Response(serializer.errors, status=status.HTTP_400_BAD_REQUEST)
        except Deparazitare.DoesNotExist:
            return Response({"error": "Deparazitare not found!"}, status=status.HTTP_404_NOT_FOUND)
    def destroy(self, request, pk=None):
        try:
            deparazitare = Deparazitare.objects.get(pk=pk)
            deparazitare.delete()
            return Response({"message": "Deparazitare deleted successfully!"}, status=status.HTTP_204_NO_CONTENT)
        except Deparazitare.DoesNotExist:
            return Response({"error": "Deparazitare not found!"}, status=status.HTTP_404_NOT_FOUND)
